...
// 	const maxCap = 500
// 	for _, handle := range existedHandles {
// 		subsFromEpoch, errs := FetchAtcoderSubmissionListFrom(handle, time.Unix(0, 0).Unix())
// 		if errs != nil {
// 			t.Error(errs)
// 		}
// 		subs, errs := FetchAtcoderSubmissionList(handle)
// 		if errs != nil {
// 			t.Error(errs)
// 		}
...
func TestAtcoderUser(t *testing.T) {
	...
	for _, handle := range unexistedHandles {
		tests = append(tests, struct {
			...
			handle:  handle,
			wantErr: usererr.ErrUserNotFound(handle),
		})
	}
